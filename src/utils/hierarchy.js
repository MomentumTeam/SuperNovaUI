...
export const getHierarchy = (hierarchy) => {
  ...
  const hierarchyReadOnly = splitHierarchy
    .slice(0, splitHierarchy.length - 1)
    .join('/');
...
export const processHierarchyData = async (hierarchy) => {
  ...
  return await Promise.all(
    hierarchy.directRoles.map(async (role) => {
      let newRow = {};
...
export const renameObjKeys = (arr, replaceKeys) => {
  return arr.map((item) => {
    const newItem = {};
