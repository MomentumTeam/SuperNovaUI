    ...
    const renderFooter = () => {
        return (
            <div className="display-flex display-flex-end">
                ...
                <Button label="שמירה" onClick={hideDialog} className="btn-orange-gradient" />
            </div>
        );
    }
    ...
    return (
        <ul className="display-flex units-wrap">
            {actionList.map(({ id, className, actionName }) => (
                <li key={id}>
                    <Button className={className} title={actionName} label={actionName} onClick={showDialog} />
                </li>
            ))}
            <Dialog header="תפקיד חדש" visible={isDialogShown} onHide={hideDialog} footer={renderFooter()}>
                <ModalForm />
            </Dialog>
        </ul>
    );
