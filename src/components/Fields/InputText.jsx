...
const InputTextField = ({
  ...
  const { item, isEdit, changeForm, errors, changeErrors } =
  ...
  return (
    <div className={`p-fluid-item ${additionalClass}`}>
      <div className='p-field'>
          ...
          onChange={(e) => {
            const errormsg =
              e.target.value === ''
                ...
                : null;
            errormsg
              ? changeErrors(fieldName, errormsg)
              : changeErrors(fieldName);
        ...
        {errors !== null && errors[fieldName] !== null && (
          <small className='p-error p-d-block'>{errors[fieldName]}</small>
        )}
      </div>
    </div>
  );
