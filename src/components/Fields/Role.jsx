...
import { getIsJobTitleAlreadyTaken } from '../../service/KartoffelService';
...
const RoleField = ({ role, isEdit, setIsJobTitleFree }) => {
  ...
  const searchRole = async (event) => {
    ...
    if (event.query !== undefined) {
      const result = await getIsJobTitleAlreadyTaken(
        event.query,
        role.directGroup
      );
  ...
  return (
    <div>
      {isEdit && (
        <div className={`status ${isJobTitleAlreadyTaken ? '' : 'available'}`}>
      ...
      />
      {errors[fieldName] &&
        errors[fieldName].type !== 'jobTitle-valid-check' && (
